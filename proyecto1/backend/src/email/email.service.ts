...
export class EmailService {
  ...
  async sendConfirmationEmail(email: string, ticket: any) {
  ...
  private async generateReservationEmail(ticket: any): Promise<{ html: string, qrBuffer: Buffer }> {
  ...
  private async generatePaidTicketEmail(ticket: any): Promise<{ html: string, qrBuffers: Buffer[] }> {
    const ticketDetails = ticket.ticketSummary?.ticketDetails || [];
    const totalAmount = ticket.ticketSummary?.totalAmount || 0;
    
    // Generar QR individual para cada entrada
    const ticketQRs = await Promise.all(
      ticketDetails.map(async (detail, index) => {
